...
func _physics_process(delta: float) -> void:
	...
	if state == "walking":
		...
		self.look_at(self.get_parent().get_parent().get_node("player").position)

		if self.position.distance_to(self.get_parent().get_parent().get_node("player").position) < 1.0:
			attack()
...
func update_pos():
	$NavigationAgent3D.target_position = self.get_parent().get_parent().get_node("player").position
